...
- [ ] `azure-search-documents` ライブラリ追加（requirements.txt）
- [ ] `backend/config.py` にAzure Search設定追加（既存設定拡張）
- [ ] 接続テスト・ヘルスチェック機能（`scripts/test_azure_search.py`）
- [ ] 環境変数設定検証（`.env.sample` 更新）
- **完了条件**: Azure AI Search接続成功、基本検索クエリ実行可能
- **所要時間**: 25分
- **🔄 コミット**: `feat(backend): setup Azure AI Search client connection`

**Task 3-1A-2: SearchServiceクラス実装**
- [ ] `backend/services/search_service.py` 作成（既存services構造準拠）
- [ ] `ISearchService` インターフェース定義（RAGアーキテクチャ準拠）
- [ ] 基本検索機能実装（ハイブリッド検索対応）
- [ ] エラーハンドリング・ログ設定（既存パターン準拠）
- [ ] `backend/deps.py` に依存注入設定追加
- **完了条件**: SearchService基本機能、DIコンテナ統合
- **所要時間**: 35分
- **🔄 コミット**: `feat(backend): implement SearchService class`

**Task 3-1A-3: 検索インデックス設計・作成**
- [ ] インデックススキーマ定義（id, title, content, chunk_id, metadata, embeddings）
- [ ] フィールドマッピング設定（searchable, filterable, retrievable）
- [ ] 日本語アナライザー設定（ja.microsoft, ja.lucene）
- [ ] ベクトル検索設定（embeddings field configuration）
- [ ] `scripts/create_search_index.py` 作成
- [ ] インデックス削除・再作成機能
- **完了条件**: 検索インデックス作成成功、スキーマ確認
- **所要時間**: 50分
- **🔄 コミット**: `feat(backend): create search index schema and configuration`
...
- [ ] 処理パイプラインテスト
- [ ] GraphQL APIテスト
- **完了条件**: 新機能のユニットテスト >80%カバレッジ
- **所要時間**: 40分
- **🔄 コミット**: `test: add unit tests for AI Search integration`
