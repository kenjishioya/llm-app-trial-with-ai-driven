...
class EnvironmentValidator:
    ...
    def log_issue(self, category: str, field: str, message: str, severity: str = "error"):
        """問題を記録"""
        issue = {
            "category": category,
            "field": field,
        ...
        for env_file in env_files:
            ...
            if env_path.exists():
                ...
                if env_file == ".env.sample":
                    self.log_issue("file_access", env_file, "サンプルファイルが存在しません", "warning")
                elif env_file != ".env.production":  # 本番環境ファイルは存在しなくても良い
    ...
    def validate_database_config(self, settings: Settings) -> bool:
        ...
        try:
            ...
            if db_info['scheme'] == 'postgresql+asyncpg':
                if not db_info['hostname']:
                    self.log_issue("database", "hostname", "PostgreSQLホスト名が設定されていません")
                    success = False
                if not db_info['database']:
                    self.log_issue("database", "database", "データベース名が設定されていません")
                    success = False

                self.info.append(f"✅ PostgreSQL: {db_info['hostname']}:{db_info['port']}/{db_info['database']}")
        ...
        if settings.db_pool_size < 1:
            self.log_issue("database", "pool_size", "接続プールサイズが小さすぎます", "warning")
        elif settings.db_pool_size > 50:
            self.log_issue("database", "pool_size", "接続プールサイズが大きすぎます", "warning")
        else:
            self.info.append(f"✅ 接続プール: {settings.db_pool_size}-{settings.db_pool_size + settings.db_max_overflow}")
    ...
    def validate_api_keys(self, settings: Settings) -> bool:
        ...
        for api_name, is_configured in api_status.items():
            if is_configured:
                ...
                severity = "error" if settings.is_production() and api_name == "openrouter" else "warning"
                self.log_issue("api_keys", api_name, f"{api_name.upper()} APIキーが設定されていません", severity)
                if severity == "error":
                    success = False
        ...
        if settings.is_production():
            if not settings.openrouter_api_key:
                self.log_issue("api_keys", "openrouter", "本番環境では必須のAPIキーです")
                success = False

        # 開発環境推奨チェック
        elif settings.is_development():
            if not any(api_status.values()):
                self.log_issue("api_keys", "general", "開発環境でもAPIキー設定を推奨します", "warning")
    ...
    def validate_security_config(self, settings: Settings) -> bool:
        ...
        if len(settings.jwt_secret_key) < 32:
            self.log_issue("security", "jwt_secret", "JWTシークレットキーが短すぎます（32文字以上推奨）")
            success = False
        else:
        ...
        if len(settings.session_secret_key) < 32:
            self.log_issue("security", "session_secret", "セッションシークレットキーが短すぎます（32文字以上推奨）")
            success = False
        else:
        ...
        if settings.is_production():
            if settings.debug:
                self.log_issue("security", "debug", "本番環境でデバッグモードが有効です")
                success = False

            if "localhost" in settings.allowed_origins:
                self.log_issue("security", "cors", "本番環境でlocalhostが許可されています", "warning")

            # デフォルトキーチェック
            default_keys = ["dev-jwt-secret-change-in-production", "dev-session-secret-change-in-production"]
            if settings.jwt_secret_key in default_keys:
                self.log_issue("security", "jwt_secret", "本番環境でデフォルトJWTキーが使用されています")
                success = False

            if settings.session_secret_key in default_keys:
                self.log_issue("security", "session_secret", "本番環境でデフォルトセッションキーが使用されています")
                success = False
    ...
    def validate_network_config(self, settings: Settings) -> bool:
        ...
        for origin in origins:
            if not origin.startswith(('http://', 'https://')):
                self.log_issue("network", "cors", f"無効なオリジン形式: {origin}", "warning")
    ...
    def generate_fix_suggestions(self, settings: Optional[Settings] = None) -> List[str]:
        """修正提案生成"""
        suggestions = []
        ...
        if self.issues:
            ...
            for issue in self.issues:
                ...
                if category == "api_keys" and "設定されていません" in message:
                    ...
                    suggestions.append(f"   💡 .env.developmentに{api_name}_API_KEY=your_key_hereを追加")

                elif category == "security" and "短すぎます" in message:
                    suggestions.append("   💡 openssl rand -base64 32 でランダムキーを生成")

                elif category == "database" and "ホスト名" in message:
                    suggestions.append("   💡 DATABASE_URL=postgresql+asyncpg://user:pass@host:5432/dbname を設定")  # pragma: allowlist secret
        ...
        if self.warnings:
            ...
            for warning in self.warnings:
                suggestions.append(f"⚠️  [{warning['category'].upper()}] {warning['field']}: {warning['message']}")

        return suggestions

    def run_validation(self, detailed: bool = False, fix_suggestions: bool = False) -> bool:
        """包括的検証実行"""
        ...
        if self.warnings:
            logger.info("⚠️  警告:")
            for warning in self.warnings:
                logger.warning(f"  [{warning['category'].upper()}] {warning['field']}: {warning['message']}")
            logger.info("")

        if self.issues:
            logger.error("❌ エラー:")
            for issue in self.issues:
                logger.error(f"  [{issue['category'].upper()}] {issue['field']}: {issue['message']}")
            logger.info("")
